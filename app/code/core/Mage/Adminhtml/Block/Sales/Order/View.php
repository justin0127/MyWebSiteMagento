...
class Mage_Adminhtml_Block_Sales_Order_View extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        if (1) {
            $this->_addButton('order_resend', array(
                'label'     => Mage::helper('sales')->__('Resend to Arvato'),
                'onclick'   => 'setLocation(\'' . $this->getResendUrl() . '\')',
            ));
        }
        if (1) {
            $this->_addButton('order_obflag', array(
                'label'     => Mage::helper('sales')->__('已外呼(OB)'),
                'onclick'   => 'setLocation(\'' . $this->getObflagUrl() . '\')',
            ));
        }
    ...
    public function getObflagUrl()
    {
        return $this->getUrl('*/*/obflag');
    }
