		...
		$customers = Mage::getModel('customer/customer')->getCollection()
	   	...
	   	if (count($customers) > 0) {
			foreach ($customers as $customer) {
				if ($customer->getConfirmation() != NULL && $customer->getPosId() != NULL) {
					$_customers = Mage::getModel('customer/customer')->getCollection()
						->addAttributeToSelect('*')
						->addFieldToFilter('email', $email);
					if (count($_customers) > 0) {
						foreach ($_customers as $_customer) {
							$_CustomerName = $_customer->getFirstname().$_customer->getLastname();
						}
							if ($_CustomerName == $firstname.$lastname) {
