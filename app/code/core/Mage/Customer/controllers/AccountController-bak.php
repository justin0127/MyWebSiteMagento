...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function saveXianxia($PostInfo)
    {
	if(!$PostInfo['message']) {   //激活码为空
		...
		$BackendMessage = $customer->getMessage();   //后台存储的信息
		if ($CurrentMessage != $BackendMessage) {
			$session->addError($this->__('Confirm Message Error.'));   //激活码无效
		}else{
    ...
    public function generalLogin($session)
    {
    ...
    public function yanzheng($session)
    {
    ...
    public function loginPostAction()
    {
		...
		if (array_key_exists('message', $PostInfo)) {    //激活登录
			$this->saveXianxia($PostInfo);
		}else{
		/*  end op1  */
        		$this->yanzheng($session);  //现在激活需要验证
            	}
        }
    ...
    public function generalcreatePost($session, $PostInfo)
    {
    ...
    public function getxianxiaName()
    {
    ...
    public function createPostAction()
    {
        ...
        /*  op2  */
        $GetEmail = $this->getRequest()->getPost('verify');   //激活
        
        if ($GetEmail == "true") {    //激活操作
		...
		if ($MdNum != $MdCookie) {    //验证码不符
			...
			//if(($CustomerMobile != NULL)&&($this->getxianxiaName() != $CustomerName)&&($Type == 'offline')) {   //手机姓名验证不符
			//	$message = $this->__('会员姓名与手机号不符，请再次确认！');
			//	$session->setEscapeMessages(false);
			//	$session->addError($message);
			//	$this->_redirectError(Mage::getUrl('*/*/create?regist_type=offline', array('_secure' => true)));
			//}else{    //通过手机姓名验证
				$this->generalcreatePost($session, $PostInfo);
				if($this->getRequest()->getPost('regist_type') == 'offline') {
					$this->_redirectError(Mage::getUrl('*/*/create?regist_type=offline', array('_secure' => true)));
				}else{
					$this->_redirectError(Mage::getUrl('*/*/create', array('_secure' => true)));
