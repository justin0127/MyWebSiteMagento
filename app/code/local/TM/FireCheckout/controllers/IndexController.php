...
//            $this->getCheckout()->getQuote()->setCustomerComment($this->getRequest()->getPost('order-comment'));
		...
		$shipping_time_chinese = array('all'=>'送货时间：工作日、双休日与假日均可送货', 'holidays'=>'仅双休日、假日送货(工作日不用送货)','workdays'=>'仅工作日送货(双休日、假日不用送货)','nights'=>'送货时间：学校地址');
		if (array_key_exists($shipping_time, $shipping_time_chinese)) {
                ...
                $this->getCheckout()->saveOrder($shipping_time, $fapiao);
